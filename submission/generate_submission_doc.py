"""
Generates the submission document (Markdown and .docx) matching the Darukaa.Earth
Hackathon submission requirements.
"""
import os
import sys

def generate_submission_markdown(
    github_url: str = "https://github.com/RatishPatil37/Prakriti-AI",
    live_demo_url: str = "https://darukaa-earth-ai.vercel.app"
) -> str:
    content = f"""# Darukaa.Earth — AI Biodiversity Intelligence Chatbot Challenge
...
### 2. Live Demo URL
- **Production Web Application**: [{live_demo_url}]({live_demo_url})
- **Backend Health Check**: `{live_demo_url.replace('.vercel.app', '-api.onrender.com')}/health`
- **Readiness Probe**: `{live_demo_url.replace('.vercel.app', '-api.onrender.com')}/ready`
...
- **IDOR Protection**: `GET /api/v1/sources/{id}` strictly enforces tenant access boundaries.
...
if __name__ == "__main__":
    out_path = os.path.join(os.path.dirname(__file__), "SUBMISSION_OVERVIEW.md")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(generate_submission_markdown())
    print(f"Submission overview written to {out_path}")
